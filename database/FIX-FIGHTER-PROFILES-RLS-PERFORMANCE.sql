-- ============================================================================
-- FIX: fighter_profiles RLS Performance Issue
-- ============================================================================
...
-- Step 1: Check current policies on fighter_profiles
SELECT 
...
DECLARE
  ...
  role_list TEXT;
BEGIN
  -- Loop through all policies on fighter_profiles that need optimization
  FOR policy_rec IN
    SELECT 
      policyname,
      cmd,
  ...
  LOOP
    ...
    -- Remove curly braces and convert to proper format
    ...
    IF policy_rec.cmd = 'SELECT' THEN
      EXECUTE format(
        'CREATE POLICY %I ON public.fighter_profiles FOR SELECT TO %s USING (%s)',
    ...
    ELSIF policy_rec.cmd = 'INSERT' THEN
      EXECUTE format(
        'CREATE POLICY %I ON public.fighter_profiles FOR INSERT TO %s WITH CHECK (%s)',
        policy_rec.policyname,
        role_list,
    ...
    ELSIF policy_rec.cmd = 'UPDATE' THEN
      EXECUTE format(
        'CREATE POLICY %I ON public.fighter_profiles FOR UPDATE TO %s USING (%s) WITH CHECK (%s)',
        policy_rec.policyname,
        role_list,
        COALESCE(new_qual, 'true'),
    ...
    ELSIF policy_rec.cmd = 'DELETE' THEN
      EXECUTE format(
        'CREATE POLICY %I ON public.fighter_profiles FOR DELETE TO %s USING (%s)',
        policy_rec.policyname,
        role_list,
        COALESCE(new_qual, 'true')
      );
