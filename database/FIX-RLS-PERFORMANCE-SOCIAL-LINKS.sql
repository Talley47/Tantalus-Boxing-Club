-- ============================================================================
-- FIX: RLS Performance Issue on social_links table
-- ============================================================================
-- Issue: RLS policy "Users can insert their own social links" uses
...
-- Step 1: Find all policies on social_links table that need optimization
SELECT 
...
-- Users can insert their own social links (OPTIMIZED)
...
-- Users can update their own social links (OPTIMIZED)
...
-- Users can delete their own social links (OPTIMIZED)
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE schemaname = 'public' 
      AND tablename = 'social_links'
      AND policyname = 'Users can insert their own social links'
  ...
  SELECT EXISTS (
    ...
    WHERE schemaname = 'public' 
      AND tablename = 'social_links'
      AND policyname = 'Users can update their own social links'
  ...
  SELECT EXISTS (
    ...
    WHERE schemaname = 'public' 
      AND tablename = 'social_links'
      AND policyname = 'Users can delete their own social links'
      AND qual LIKE '%(select auth.uid())%'
  ...
  IF optimized_insert_policy_exists 
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'All RLS policies on social_links table are now optimized!';
    RAISE NOTICE 'Performance warning should be resolved.';
  ELSIF optimized_insert_policy_exists THEN
    RAISE WARNING '⚠️  Insert policy optimized, but other policies may need optimization';
...
-- FIX COMPLETE
-- ============================================================================
-- The "Users can insert their own social links" policy (and related policies)
