...
2. **Copy the SQL below** (everything between the lines)
...
1. **Creates a database function** `create_auto_matched_fight()` that bypasses RLS
2. **Grants permissions** so authenticated users can call it
3. **Updates RLS policies** to allow manual fight creation
4. **Verifies** the function was created successfully

## After Running:

- The "Mix and Match Fighters" button will work
- Auto-matched fights will be created without RLS errors
- Manual fight creation will still work normally

---

## ⚠️ Important:

**DO NOT copy error messages from the browser console!** Only copy the SQL code above.

If you see an error like:
```
fetch.ts:7 POST https://...supabase.co/rest/v1/scheduled_fights?select=* 403 (Forbidden)
```

That's a JavaScript error message, NOT SQL. Ignore it and use the SQL script above instead.
