...
-- Title Belts
CREATE TABLE IF NOT EXISTS title_belts (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    weight_class VARCHAR(30) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_fighter_profiles_handle ON fighter_profiles(handle);
CREATE INDEX IF NOT EXISTS idx_fighter_profiles_tier ON fighter_profiles(tier);
...
CREATE INDEX IF NOT EXISTS idx_matchmaking_requests_requester ON matchmaking_requests(requester_id);
CREATE INDEX IF NOT EXISTS idx_matchmaking_requests_status ON matchmaking_requests(status);

CREATE INDEX IF NOT EXISTS idx_scheduled_fights_fighter1 ON scheduled_fights(fighter1_id);
CREATE INDEX IF NOT EXISTS idx_scheduled_fights_fighter2 ON scheduled_fights(fighter2_id);
CREATE INDEX IF NOT EXISTS idx_scheduled_fights_date ON scheduled_fights(scheduled_date);

CREATE INDEX IF NOT EXISTS idx_notifications_user_id ON notifications(user_id);
CREATE INDEX IF NOT EXISTS idx_notifications_is_read ON notifications(is_read);

CREATE INDEX IF NOT EXISTS idx_media_assets_fighter_id ON media_assets(fighter_id);
...
DROP POLICY IF EXISTS "Users can update their own fighter profile" ON fighter_profiles;
CREATE POLICY "Users can update their own fighter profile" ON fighter_profiles
    FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own fighter profile" ON fighter_profiles;
...
-- Insert default tiers (skip if they already exist)
...
('matchmaking_enabled', 'true'::jsonb, 'Enable/disable matchmaking system'),
('tier_promotion_threshold', '50'::jsonb, 'Points required for tier promotion'),
('tier_demotion_threshold', '4'::jsonb, 'Consecutive losses before tier demotion'),
('dispute_timeout_hours', '24'::jsonb, 'Hours before dispute auto-resolution'),
('tournament_entry_fee_percentage', '0.1'::jsonb, 'Percentage of prize pool as entry fee'),
('max_fighters_per_tournament', '32'::jsonb, 'Maximum fighters allowed in tournaments'),
